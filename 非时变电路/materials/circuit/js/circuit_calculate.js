...
function powerconnector() {
    ...
    for (var i = 0; i < circuitState.elements.length; i++) {
        if (circuitState.elements[i] instanceof PowerVDC) {
            //找到电源
            for (var j = 0; j < 2; j++) {
                var powerconnect = [];
                powerconnect.push(
...
/**剔除虚假节点 o(nN)*/
function finalnodelist() {
    var connectormatrix = new Array(circuitState.connectors.length).fill(0),
        fakenode = [],
        linkmatrix = getlinkmatrix(),
        nodelist = powerconnector();
    for (var i1 = 0; i1 < circuitState.connectors.length; i1++) {
        //找出无连接的接线柱
...
function checknodelist() {
    var nodelist = powerconnector(),
        ...
        n = 0;
    for (var i = 0; i < circuitState.elements.length; i++) {
        if (circuitState.elements[i].brokenType == 2) {
            n = circuitState.elements[i].connectors[0];
        } else if (circuitState.elements[i].brokenType == 3) {
            n = circuitState.elements[i].connectors[1];
        }
...
function getbranches() {
    ...
    for (var i = 0; i < nodelist.length; i++) {
        ...
        for (var j = 0; j < linknolist.length + 1; j++) {
            //遍历与节点相邻的接线柱
            var branch = {};
            (branch.node = []), (branch.elements = []), (branch.wires = []);
            if (j < linknolist.length) {
                //对单元件支路的特判
                k = linknolist[j];
    ...
    for (var i2 = 0; i2 < branches.length; i2++) {
        if (fakenode.includes(branches[i2].node[0])) {
            for(var i3=0;i3<branches[i2].elements.length&&branches[i2].elements!=undefined;i3++){
                if (
                    branches[i2].elements[i3] instanceof Ammeter ||
                    branches[i2].elements[i3] instanceof Voltmeter
                )
                {
                branches[i2].elements[i3].U=0;
                branches[i2].elements[i3].I=0;
            }
            }
            branches.splice(i2, 1);
            i2--;
        } else if (
            fakenode.includes(branches[i2].node[branches[i2].node.length - 1])
        ) {
            for(var i3=0;i3<branches[i2].elements.length&&branches[i2].elements!=undefined;i3++){
                if (
                    branches[i2].elements[i3] instanceof Ammeter ||
            ...
            branches[i2].node[0] ==
            branches[i2].node[branches[i2].node.length - 1]
        ) {
            for(var i3=0;i3<branches[i2].elements.length&&branches[i2].elements!=undefined;i3++){
                if (
                    branches[i2].elements[i3] instanceof Ammeter ||
    ...
    for (var n1 = 0; n1 < branches.length; n1++) {
        //将导线加入支路
        for (var n2 = 0; n2 < branches[n1].node.length; n2++) {
            for (var n3 = 0; n3 < wirematrix.length; n3++) {
                t1 = n2 + 1;
                if (
                    branches[n1].node[n2] == wirematrix[n3].loc1.No &&
                    branches[n1].node[t1] == wirematrix[n3].loc2.No
                ) {
                    branches[n1].wires.push(wirematrix[n3]);
                } else {
                    if (
                        branches[n1].node[n2] == wirematrix[n3].loc2.No &&
                        branches[n1].node[t1] == wirematrix[n3].loc1.No
                    ) {
                        branches[n1].wires.push(wirematrix[n3]);
                    }
                }
...
function Gdiag() {
    ...
    for (var k = 0; k < nG; k++) {
        ...
        for (var k1 = 0; k1 < branchmatrix[k].elements.length; k1++) {
            if (branchmatrix[k].elements[k1].brokenType == -1) {
                g = g + branchmatrix[k].elements[k1].R;
            } else if (branchmatrix[k].elements[k1].brokenType == 0) {
                g = g + 0;
            } else if (branchmatrix[k].elements[k1].brokenType == 1) {
                g = g + Infinity;
            } else if (
                branchmatrix[k].elements[k1].brokenType == 2 &&
                Bnode.includes(branchmatrix[k].elements[k1].connectors[0]) ==
                    false
            ) {
                g = g + 0.4;
            } else if (
                branchmatrix[k].elements[k1].brokenType == 3 &&
                Bnode.includes(branchmatrix[k].elements[k1].connectors[1]) ==
                    false
            ) {
                g = g + 0.4;
            }
        ...
        for (var k2 = 0; k2 < branchmatrix[k].wires.length; k2++) {
            if (branchmatrix[k].wires[k2].brokenType == 0) {
                g = g + 0.5;
            } else if (branchmatrix[k].wires[k2].brokenType == 1) {
                g = g + Infinity;
            } else if (branchmatrix[k].wires[k2].brokenType == -1) {
        ...
        //节点接线柱损坏带来的阻值增加
        if (Bnode.includes(branchmatrix[k].node[0])) {
            g = g + 0.2;
        }
        if (
            Bnode.includes(
                branchmatrix[k].node[branchmatrix[k].node.length - 1]
            )
        ) {
            g = g + 0.2;
        }
...
function Usmatrix() {
    ...
    for (i1 = 0; i1 < associationmatrix[0].length; i1++) {
        if (branchmatrix[i1].elements[0] instanceof PowerVDC) {
            if(branchmatrix[i1].elements[0].ON==true){
            ...
            if (branchmatrix[i1].elements[0].brokenType == 0) {
                Us[i1] = Us[i1] * 0.96; //电压偏低
            } else if (branchmatrix[i1].elements[0].brokenType == 1) {
...
function Ismatrix() {
    var Is = [],
        ...
        t2 = 0;
    for (i1 = 0; i1 < associationmatrix[0].length; i1++) {
        if (branchmatrix[i1].elements[0] instanceof PowerIDC) {
            for (i2 = 0; i2 < branchmatrix[i1].node.length; i2++) {
                if (
                    branchmatrix[i1].elements[0].connectors[0] ==
                    branchmatrix[i1].node[i2]
...
function calculate() {
    ...
    if(C21.length==0){
        CalculateResult.U =zero(AT.length,1)
        CalculateResult.Us = zero(A.length,1);
...
function checkdirection() {
    ...
    for (i1 = 0; i1 < associationmatrix[0].length; i1++) {
        for (i2 = 0; i2 < branchmatrix[i1].elements.length; i2++) {
            if (
                branchmatrix[i1].elements[i2] instanceof Ammeter ||
...
function getResults() {
    ...
    for (var i = 0; i < branchmatrix.length; i++) {
        for (j = 0; j < branchmatrix[i].elements.length; j++) {
            ...
            if (branchmatrix[i].elements[j] instanceof PowerVDC) {
                // do nothing
